...
void Page::showButtons(bool show){
  for (uint8_t buttonNum = 0; buttonNum < 5; ++buttonNum){
    ...
    if (show){
      ...
      mainPage.showButtonSprite(buttonPressed[buttonNum], buttonNum, controls[controlNum].actionName, controls[controlNum].color, controls[controlNum].luminance);
    }
    else {
      mainPage.showButtonSprite(false, buttonNum, " ", defaultBgColor, 0);
    }
  }
}
...
void Page::showLeftMarkerName()
{
  displayItemByType(LeftMarkerName, mainParser.leftMarkerName);
}

void Page::showRightMarkerName()
{
  displayItemByType(RightMarkerName, mainParser.rightMarkerName);
}
...
void Page::updatePlaySprite()
{
  if (pageType == SETLIST)
  {
    ...
    if (global.isPlaying)
      setRGBColor(1, 0, 255, 0);
    else
      pages[0].setRGBColor(1, 0, 40, 0);
    l[1].show_color();
  ...
  for (int i = 0; i < 3; ++i)
  {
    if (displayedItem[i] == itemType)
    {
      ...
      if (pageType == SETLIST)
      {
        ...
        {
        case 0:
          showMainSprite(content, defaultTxtColor);
          break;
        case 1:
          showSub1Sprite(content, defaultTxtColor);
          break;
        case 2:
          showSub2Sprite(content, defaultTxtColor);
          break;
        }
      }
      ...
      else if (pageType == USER)
      {
        ...
        {
        case 0:
          showMainUserSprite(content, defaultTxtColor);
          break;
        case 1:
          showSub1UserSprite(content, defaultTxtColor);
          break;
