...
// Validation robuste pour les valeurs displayedItem dans les pages USER
static bool isValidDisplayedItem(displayed_item_t item) {
  // Vérification que la valeur est dans les limites de l'enum
  return item >= Empty && item <= Active_Song_Seconds;
}
...
void Page::showPage() {
  ...
  if (isSetlist) {
    ...
    // Affichage des 3 sprites selon displayedItem avec validation robuste
    for (int i = 0; i < 3; ++i) {
      DEBUG_LOG_VALUE("Processing displayedItem: ", i);
      
      // PROTECTION ROBUSTE: Validation de la valeur displayedItem
      displayed_item_t currentItem = displayedItem[i];
      if (!isValidDisplayedItem(currentItem)) {
        DEBUG_LOG_VALUE("Invalid displayedItem value detected: ", (int)currentItem);
        DEBUG_LOG_VALUE("Setting to default (Empty) for slot: ", i);
        currentItem = Empty; // Valeur par défaut sécurisée
      }
...
void Page::showMainSprite(const char* txt, uint16_t color) {
  if(pageType == SETLIST) showSprite(txt, color, mainSprite);
}
...
void Page::showMainUserSprite(const char* txt, uint16_t color) {
  if(pageType == USER) showSprite(txt, color, mainUserSprite);
}
